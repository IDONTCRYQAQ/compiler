...
%{
#define LIST     linebuf += std::string(yytext)
...
%{
  ...
  # define YY_USER_ACTION  { if(yytext[0]!= '\n' && !str_state) LIST; loc.columns (yyleng); }
%}
...
\"  { yymore(); str_state = true; linebuf.pop_back(); BEGIN(STR); }    //  string (left quotation mark)
...
<CMT>\n { line_num++; linebuf.clear(); 
          linebuf.clear(); loc.lines(yyleng); loc.step();  
        }
...
<<EOF>> { drv.fout<< "/* " << line_num++ << ": " << linebuf << " */" << std::endl; 
          linebuf.clear(); return yy::parser::make_YYEOF(loc); }
