...
public:
    ...
    void exit_block();
    //  -----------------------------------------------------------    //
    bool in_global() { return data_environment_.in_global(); }
    bool is_global(const std::string& id) { return data_environment_.is_global(id); }
    ...
    int get_number(const std::string& id) const { return data_environment_.get_number(id); }
...
private:
    ...
    public:
        ...
        bool in_global() { return scopes_.size() == 1; }
        bool is_global(const std::string& id) const;

        bool exist(const std::string& id) const;
        bool is_declared(const std::string& id) const;
        const DataType& get_data_type(const std::string& id) const;
        Data get_data(const std::string& id);
        int get_number(const std::string& id) const { return number_.at(id).back(); }
        void default_variable(const std::string& id, ValueType type = ValueType::INTEGER);
    private:
        ...
        // local variable
        std::unordered_map< std::string, std::vector<int> > number_;
        std::vector< std::vector<std::string> > dump_list_;
        int count = 0; 
        //
